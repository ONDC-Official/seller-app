...
class ProductController {
    ...
    async searchIncrementalPull(req, res, next) {
        try {
            ...
            query.offset = 0;
            query.limit = 50; //default only 50 products will be sent
            const products = await productService.searchIncrementalPull(
                query,
                req.params.category
            );
            return res.send(products);
    ...
    async ondcGetForUpdate(req, res, next) {
        try {
            const params = req.params;
            const product = await productService.ondcGetForUpdate(
                params.productId
            );
            return res.send(product);
        } catch (error) {
    ...
    async getWithVariants(req, res, next) {
        try {
            const params = req.params;
            const product = await productService.getWithVariants(
                params.productId,
                req.user
            );
            return res.send(product);
    ...
    async updateWithVariants(req, res, next) {
        try {
            // const params = req.params;
            const product = await productService.updateWithVariants(
                req.body,
                req.user
            );
            return res.send(product);
    ...
    async uploadTemplate(req, res, next) {
        try {
            ...
            const filePath = `app/modules/product/template/${category
                .toLowerCase()
                .replace(/\s+/g, "_")}.xlsx`;
            ...
            fs.access(filePath, fs.constants.F_OK, (err) => {
                if (err) {
                    return res.status(404).json({
                        success: false,
                        message:
                            "Template not found for the specified category",
    ...
    async categorySubcategoryAttributeList(req, res, next) {
        try {
            ...
            const categoryVariant =
                await productService.categorySubcategoryAttributeList(params);
    ...
    async categorySubcategoryList(req, res, next) {
        try {
            const params = req.query;
            const categoryVariant =
                await productService.categorySubcategoryList(params);
            return res.send(categoryVariant);
        } catch (error) {
            next(error);
        }
    ...
    async getCustomizations(req, res, next) {
        try {
            const params = req.params;
            const categoryVariant = await productCustomizationService.get(
                params.productId,
                req.user
            );
            return res.send(categoryVariant);
    ...
    async storeCustomizations(req, res, next) {
        try {
            const params = req.params;
            const data = req.body;
            const categoryVariant = await productCustomizationService.create(
                params.productId,
        ...
        try {
            ...
            if (jsonData.length === 0) {
                ...
                if (
                    ...
                    inputKeys.every((e) => !validKeys.includes(e))
                ) {
                    return res.status(400).json({
                        success: false,
                ...
                const mergedSchema = mergedValidation(
                    category.toLowerCase().replace(/\s+/g, "")
                );
                const commonSchema = mergerdAttributeValidation(
                    category.toLowerCase().replace(/\s+/g, "")
                );
                for (const row of jsonData) {
                    ...
                    const commonKeys = Object.keys(row).filter((key) =>
                        templateAttributeKeys[
                            category.toLowerCase().replace(/\s+/g, "")
                        ].includes(key)
                    );
                    const commonRow = {};
                    ...
                    const {
                        ...
                        value: validatedCommonRow,
                    } = commonSchema.validate(commonRow, {
                        allowUnknown: true, // Validate common attributes separately
                    });

                    // Validate merged schema for the row
                    const { error: validationError, value: validatedRow } =
                        mergedSchema.validate(row, {
                    ...
                    if (!commonValidationError && !validationError) {
                        ...
                        if (commonValidationError) {
                            errors = errors.concat(
                                commonValidationError.details.map(
                                    (err) => err.message
                                )
                            );
                        }
    ...
    async createCustomization(req, res, next) {
        try {
            ...
            const result = await productService.createCustomization(
                data,
                req.user
            );
    ...
    async getCustomization(req, res, next) {
        try {
            ...
            const customizations = await productService.getCustomization(
                params,
                req.user
            );
            ...
            console.log(
                "[CustomizationController] [getCustomization] Error:",
                error
            );
            next(error);
        }
    ...
    async updateCustomization(req, res, next) {
        try {
            ...
            const updateResult = await productService.updateCustomization(
                updatedDetails,
                req.user,
                customizationId
            );
            ...
            if (updateResult) {
                ...
                return res.status(400).json({
                    success: false,
                    message: "Failed to update customizations.",
                });
            }
        } catch (error) {
            console.log(
                "[CustomizationController] [updateCustomization] Error -",
                error
            );
            next(error);
        }
    ...
    async deleteCustomization(req, res, next) {
        try {
            const { customizationId } = req.params;
            const deleteResult = await productService.deleteCustomization(
                customizationId,
                req.user
            );
            ...
            if (deleteResult && deleteResult.success) {
                return res.status(200).json({
                    ...
                    deletedCustomization: deleteResult.deletedCustomization,
                });
            } else {
                return res.status(404).json({
                    success: false,
                    message:
                        "Failed to delete customization or customization not found.",
                });
            }
        } catch (error) {
            console.log(
                "[CustomizationController] [deleteCustomization] Error -",
                error
            );
            next(error);
        }
    ...
    async getCustomizationById(req, res, next) {
        try {
            ...
            const customization = await productService.getCustomizationById(
                customizationId,
                currentUser
            );
            ...
            console.error(
                "[CustomizationController] [getCustomizationById] Error:",
                err
            );
            next(err);
        }
