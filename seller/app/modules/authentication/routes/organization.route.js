...
router.post(
    "/v1/organizations/:id/storeDetails",
    authentication.middleware(),
    authorisation.middleware({ roles: [SYSTEM_ROLE.ORG_ADMN] }),
    apiParamsValidator.middleware({
        schema: organisationSchema.setStoreDetails(),
    }),
    organizationController.setStoreDetails
);

router.get(
    "/v1/organizations/:organizationId/storeDetails",
    authentication.middleware(),
    authorisation.middleware({ roles: [SYSTEM_ROLE.ORG_ADMN] }),
    apiParamsValidator.middleware({
        schema: organisationSchema.getStoreDetails(),
    }),
...
router.get(
    "/v1/organizations/detailsForOndc/:organizationId",
    // authentication.middleware(),
    // authorisation.middleware({roles: [SYSTEM_ROLE.SUPER_ADMIN]}),
    apiParamsValidator.middleware({ schema: organisationSchema.get() }),
    organizationController.getOrgDetailsForOndc
);
router.get(
    "/v1/organizations/detailsForOndc",
    // authentication.middleware(),
    // authorisation.middleware({roles: [SYSTEM_ROLE.SUPER_ADMIN]}),
    apiParamsValidator.middleware({
        schema: organisationSchema.getAllOrgDetailsForOndc(),
    }),
    organizationController.getAllOrgDetailsForOndc
);
...
router.get(
    "/v1/organizations/:organizationId/ondcGet",
    apiParamsValidator.middleware({ schema: organisationSchema.get() }),
