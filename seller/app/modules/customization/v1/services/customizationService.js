...
class CustomizationService {
        ...
        try {
            if (customizationDetails) {
                ...
                if (!existingGroup) {
                    ...
                    let newCustomizationGroup = new CustomizationGroup(
                        customizationGroupObj
                    );
                    ...
                    for (const customizations of customizationDetails.customizations) {
                        await this.mappingCustomizations(
                            newCustomizationGroup._id,
                            customizations
                        );
                    }
                    ...
                    throw new DuplicateRecordFoundError(
                        MESSAGES.CUSTOMIZATION_GROUP_ALREADY_EXISTS
                    );
                }
            }
        } catch (err) {
    ...
    async getCustomizationGroups(params, currentUser) {
        try {
            ...
            console.log(
                `[CustomizationService] [getCustomizationGroups] Error - ${currentUser.organization}`,
                err
            );
            throw err;
        }
    ...
    async updateCustomizationGroups(id, customizationDetails, currentUser) {
        ...
        try {
            if (customizationDetails) {
                await this.isValidTreeStructure(
                    id,
                ...
                if (existingGroup) {
                    ...
                    if (customizationDetails.seq >= existingGroup.seq) {
                        const nextGroupIds = customizationDetails.customizations
                            .map((c) => c.nextGroupId.map((ng) => ng.groupId))
                            .flat();
                ...
                await CustomizationGroupMapping.deleteMany({
                ...
                throw new NoRecordFoundError(
                    MESSAGES.CUSTOMIZATION_GROUP_NOT_EXISTS
                );
            }
        } catch (err) {
            console.log(
    ...
    async deleteCustomizationGroup(currentUser, groupId) {
        try {
            ...
            const isUsedInProducts = await Product.findOne({
                customizationGroupId: groupId,
                organization: currentUser.organization,
            });
            if (isUsedInProducts) {
                throw new ConflictError(
                    MESSAGES.CUSTOMIZATION_GROUP_EXISTS_FOR_ONE_OR_MORE_PRODUCTS
                );
            }

            // Check if the group is a child in the mapping table
            const isChildInMapping = await CustomizationGroupMapping.findOne({
                child: groupId,
                organization: currentUser.organization,
            });
            if (isChildInMapping) {
            ...
            const deletedCustomizationGroup =
                await CustomizationGroup.deleteOne({
                    _id: groupId,
                    organization: currentUser.organization,
                });
            ...
            console.log(
                `[CustomizationService] [deleteCustomizations] Error - ${currentUser.organization}`,
                err
            );
            throw err;
        }
    ...
    async getCustomizationGroupById(groupId, currentUser) {
        try {
            ...
            for (const mapping of mappings) {
                ...
                if (!customization) {
                    console.error(
                        `[CustomizationService] [getCustomizationGroupById] Error - Customization not found: ${customizationId}`
                    );
                    continue;
                }
                ...
                for (const group of mapping.groups) {
                    ...
                    if (!nextGroup) {
                        console.error(
                            `[CustomizationService] [getCustomizationGroupById] Warning - Next group not found: ${group.child}`
                        );
                        continue;
                    }
                    let groupObj = {
            ...
            console.log(
                `[CustomizationService] [getCustomizationGroupById] Error - ${currentUser.organization}`,
                err
            );
            throw err;
        }
    ...
    async mappdedData(groupId, currentUser) {
        if (groupId) {
            const mappedData = await this.getMappedCustomizationAndGroup(
                groupId,
                [],
                [],
                currentUser
    ...
    async getMappedCustomizationAndGroup(
        groupId,
        customizationGroups = [],
        customizations = [],
        currentUser
        ...
        if (!group) {
            throw new NoRecordFoundError(
                MESSAGES.CUSTOMIZATION_GROUP_NOT_EXISTS + groupId
            );
        }
        customizationGroups.push(group);
        const mappedData = await CustomizationGroupMapping.find({
            parent: groupId,
            organization: currentUser.organization,
        });
                ...
                if (customization) {
                    if (data.groups && data.groups.length > 0) {
                        for (const group of data.groups) {
                            ...
                            if (group.child) {
                                const mappedGroupdata =
                                    await this.getMappedCustomizationAndGroup(
                                        group.child,
                                        customizationGroups,
                                        customizations,
                                        currentUser
                                ...
                                customizationGroups =
                                    mappedGroupdata.customizationGroups ?? [];
                                customizations =
                                    mappedGroupdata.customizations ?? [];
                            }
                        }
                    }
                }
    ...
    async isValidTreeStructure(id, customizationDetails, currentUser) {
        ...
        const response = await this.traverseForward(
            id,
            [],
            customizations,
            [],
    ...
    async traverseForward(
        ...
        if (customizations.length > 0) {
            for (const customization of customizations) {
                if (
                    customization.nextGroupId &&
                    customization.nextGroupId.length > 0
                ) {
                    for (const group of customization.nextGroupId) {
                        const groupData = await CustomizationGroupMapping.find({
                            parent: group.groupId,
                            organization: currentUser.organization,
                        });
                        childIds = await this.traverseForward(
                            group.groupId,
                            childIds,
                            [],
                            groupData,
